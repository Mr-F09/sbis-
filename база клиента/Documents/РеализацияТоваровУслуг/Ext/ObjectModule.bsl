...
Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	//+++ создание с/ф 
	Если (Организация.ОтражатьВРегламентированномУчете) И  (НЕ Отказ) тогда
	  ...
	  Если НЕ ЗначениеЗаполнено(СФ) тогда
		 ...
		 СФ.Номер = Номер;
		 СФ.Ответственный = ПараметрыСеанса.ТекущийПользователь;
		 СФ.КодВидаОперации = "01";
		 СФ.Выставлен = Истина;
		 СФ.ДатаВыставления = Дата;
		 СФ.Записать(РежимЗаписиДокумента.Запись);
		 Сообщить("Создан документ " + СокрЛП(СФ));
		 СФ.Записать(РежимЗаписиДокумента.Проведение); 
		 
         #Если Клиент Тогда
		 Оповестить("СозданСФАвто");
	     #КонецЕсли  
	 
	  КонецЕсли;//	  Если НЕ ЗначениеЗаполнено(СФ) тогда
	  	  
    КонецЕсли;//	Если Организация.ОтражатьВРегламентированномУчете тогда
