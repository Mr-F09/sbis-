...
Функция ПодготовитьОбщиеДанныеДляПечатиКорректировочногоСчетаФактуры(СчетФактура, ЭтоПечатьУКД = Ложь) Экспорт
	...
	Если ОбщиеДанныеДляПечати <> Неопределено Тогда
		...
		ОбщиеДанныеДляПечати.Вставить("ИдентификаторГосКонтракта", УчетНДС.ТекстЗаголовкаИДГК(ДанныеСчетаФактуры.ДатаСчетФактуры) + СчетФактура.ИдентификаторГосКонтракта);
...
Функция ПечатьСчетаФактуры(ДанныеДляПечати)
	...
	Для Каждого Строчка Из ВыборкаСтрокТовары Цикл
		...
		Если (Строчка.СтавкаНДС = Перечисления.СтавкиНДС.НДС20_120) Или (Строчка.СтавкаНДС = Перечисления.СтавкиНДС.НДС22_122) Или (Строчка.СтавкаНДС = Перечисления.СтавкиНДС.НДС5_105)
...
Функция ПечатьСчетаФактуры1137(ДанныеДляПечати)
	...
	КолонкиДляСвертки = ?(ВыборкаСтрокТовары.Колонки.Найти("Контрагент") <> Неопределено, "Контрагент,", "")
		+ "Товар, ТоварНаименование, ТоварКодТНВЭД, ТоварКод, ЕдиницаИзмерения, ЕдиницаИзмеренияКод, Цена, СтавкаНДС,"
...
Функция ПечатьКорректировочногоСчетаФактуры1137(ДанныеДляПечати)
	...
	Для каждого СтрокаТабличнойЧасти Из ДанныеДляПечати.ТабличнаяЧасть Цикл 
		...
		Если ДанныеДляПечати.ДатаСчетФактуры >= УчетНДС.НачалоДействияПостановления981() Тогда
			Если НЕ ЗначениеЗаполнено(ОбластьМакета.Параметры.ТоварКодТНВЭД)
...
Функция ПечатьУниверсальногоПередаточногоДокумента(ДанныеДляПечати, ТабДокумент = Неопределено) Экспорт
	...
	Если Дата >= Дата(2024,10,1) тогда
