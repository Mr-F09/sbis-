...
//
Функция ДвоичныеДанныеБезBOM(СтрокаДанных) Экспорт
	...
	УстановитьПривилегированныйРежим(БылПривилегированныйРежим);
	...
	Если ФорматОбмена <> Перечисления.ФорматыОбменаЕГАИС.V1 Тогда
		...
		ИначеЕсли ВидДокумента = Перечисления.ВидыДокументовЕГАИС.АктПодтвержденияТТН
			...
			Если ТипЗнч(ВходныеПараметры.ДокументСсылка) = Тип("ДокументСсылка.ТТНВходящаяЕГАИС") Тогда
				ЗначенияРеквизитов = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ВходныеПараметры.ДокументСсылка, "Грузоотправитель, ФорматОбмена");
				ОрганизацияЕГАИС = ЗначенияРеквизитов.Грузоотправитель;
				ФорматОбменаТТН = ЗначенияРеквизитов.ФорматОбмена;
			Иначе
				ОрганизацияЕГАИС = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВходныеПараметры.ДокументСсылка, "Грузоотправитель");
			КонецЕсли;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ФорматОбменаТТН) Тогда
			ФорматОбмена = ФорматОбменаТТН;
		ИначеЕсли ЗначениеЗаполнено(ОрганизацияЕГАИС) Тогда
...
//
Функция ПолучитьСерверТОНаСервере() Экспорт

	Возврат Обработки.ТОСервер.Создать();
...
Функция ЗаблокироватьТранспортныйМодульДляОбмена(ТранспортныйМодуль, ИдентификаторКлиента = Неопределено) Экспорт
	...
	СтруктураКлюча.Вставить("ИдентификаторФСРАР", ТранспортныйМодуль.ИдентификаторФСРАР);
	...
	Исключение
		...
		Если НЕ ПустаяСтрока(СтрокаПоиска) Тогда
			ПозицияСимвола = Найти(ОписаниеОшибки, СтрокаПоиска);
