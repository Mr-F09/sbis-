...
Функция ОбработатьОтклонениеАннулированиеЭД(СсылкаНаЭД, ПараметрыЭД, ОтклонитьАннулирование = Ложь) Экспорт
	...
	Если ПараметрыЭД.Отклонить Тогда
		...
		Иначе
			...
			Если ЭлектронныеДокументыСлужебныйВызовСервера.ЭтоСчетФактура(СсылкаНаЭД) Тогда
...
Процедура ЗаполнитьСвойстваУведомленийЭДО(СвойстваУведомлений, СертификатыПрофиля, СвойстваПрофиля) Экспорт
	...
	КонтекстЭДО = Новый Структура;
	КонтекстЭДО.Вставить("КриптографияНаСервере", Ложь);
	КонтекстЭДО.Вставить("АвторизацияНаСервере", Ложь);
	
	ЭлектронныеДокументыСлужебныйВызовСервера.ЗаполнитьКонтестЭДО(КонтекстЭДО);
	
	Если КонтекстЭДО.КриптографияНаСервере Тогда
	...
	Иначе
		...
		РасшифроватьМаркерИзСтруктурыСертификата(ПараметрыЗапросаМаркера, КонтекстЭДО.АвторизацияНаСервере);
...
//
// Возвращаемое значение:
//   Булево    - результат аутентификации на сайте.
//
...
Процедура РаспаковатьМассивПакетовЭД(МассивПЭД) Экспорт
	...
	Для Каждого СтруктураРаспаковки Из ПараметрыРаспаковки Цикл
		...
		ДанныеРаспаковки = Неопределено;
		СтруктураРаспаковки.Свойство("ДанныеРаспаковки", ДанныеРаспаковки);
		КоличествоРаспакованныхДокументов = РаспаковатьПакетЭДНаКлиенте(СтруктураРаспаковки.ПакетЭД, СтруктураШифрования, ДанныеРаспаковки);
...
//
Функция РаспаковатьПакетЭДНаКлиенте(ПакетЭД, СтруктураШифрования, ДанныеРаспаковки)
	...
	Попытка
			...
			Иначе
				...
				Если СоответствиеФайлПараметры.ДокументСодержитПодписи
					...
					Для Каждого ДанныеПодписи Из МассивДанныхПодписей Цикл
						...
						КонецЕсли;
					КонецЦикла;
					СоответствиеФайлПараметры.ДанныеСертификатовПолучены = Истина; // если данные не получены, то
					// элемент соответствия уже в массиве на удаление и будет удален из СоответствиеФайлПараметры.
				КонецЕсли;
			КонецЕсли;
		...
		Если ЕстьТипДанных И ТипДанных <> "ЭД" И ТипДанных <> "Подпись" Тогда
			...
			Если ТипЗнч(МассивДобавленныхФайлов) = Тип("Массив") И МассивДобавленныхФайлов.Количество() > 0
				И НЕ ВыполнятьКриптооперацииНаСервере И ЕстьКриптосредствоНаКлиенте Тогда
			...
			Иначе
				...
				Для Каждого СтруктураДанных Из МассивДвоичныхДанных Цикл
					...
					Если Не МассивДобавленныхФайлов = Неопределено И МассивДобавленныхФайлов.Количество() > 0
						И ЕстьКриптосредствоНаКлиенте И Не ВыполнятьКриптооперацииНаСервере Тогда
