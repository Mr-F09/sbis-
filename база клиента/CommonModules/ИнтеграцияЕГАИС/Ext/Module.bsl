...
Процедура ЗапуститьОбработкуОтветовЕГАИС() Экспорт
	...
	СписокУТМ = РегистрыСведений.НастройкиОбменаЕГАИС.ДоступныеТранспортныеМодули();
...
Функция СправочникВидовАлкогольнойПродукции() Экспорт
	...
	МассивТипов = Метаданные.Справочники.КлассификаторАлкогольнойПродукцииЕГАИС.Реквизиты.ВидПродукции.Тип.Типы();
...
Функция ТекстXMLВыгрузкиДокумента(ТранспортныйМодуль, ФорматОбмена, ВидДокумента, ПараметрыЗапроса, ТекстОшибки) Экспорт
	...
	Если ВидДокумента = Перечисления.ВидыДокументовЕГАИС.ЗапросАлкогольнойПродукции Тогда
		...
		ИначеЕсли ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V2
			ИЛИ ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V3 Тогда
	...
	ИначеЕсли ВидДокумента = Перечисления.ВидыДокументовЕГАИС.ТТН Тогда
		...
		Если ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V1 Тогда
			СодержимоеXDTO.WayBill = ПреобразоватьВXDTOТТН_v1(ПараметрыЗапроса.ДокументСсылка, ТекстОшибки);
		ИначеЕсли ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V2 Тогда
			СодержимоеXDTO.WayBill_v2 = ПреобразоватьВXDTOТТН(ПараметрыЗапроса.ДокументСсылка, ФорматОбмена, ТекстОшибки);
		ИначеЕсли ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V3 Тогда
			СодержимоеXDTO.WayBill_v3 = ПреобразоватьВXDTOТТН(ПараметрыЗапроса.ДокументСсылка, ФорматОбмена, ТекстОшибки);
		КонецЕсли;
	...
	ИначеЕсли ВидДокумента = Перечисления.ВидыДокументовЕГАИС.АктПодтвержденияТТН
		...
		Если ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V1 Тогда
			СодержимоеXDTO.WayBillAct = ПреобразоватьВXDTOАктПодтвержденияТТН_v1(ВидДокумента, ПараметрыЗапроса.ДокументСсылка, ТекстОшибки);
		ИначеЕсли ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V2 Тогда
			СодержимоеXDTO.WayBillAct_v2 = ПреобразоватьВXDTOАктПодтвержденияТТН(ВидДокумента, ФорматОбмена, ПараметрыЗапроса.ДокументСсылка, ТекстОшибки);
		ИначеЕсли ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V3 Тогда
			СодержимоеXDTO.WayBillAct_v3 = ПреобразоватьВXDTOАктПодтвержденияТТН(ВидДокумента, ФорматОбмена, ПараметрыЗапроса.ДокументСсылка, ТекстОшибки);
		КонецЕсли;
	...
	Результат = СтрЗаменить(Результат, "non_qualified_element:", "");
...
Функция ПреобразоватьВXDTOАлкогольнуюПродукцию(АлкогольнаяПродукция, ТекстОшибки)
	...
	ПеречислениеВидыЛицензий = Перечисления[ИнтеграцияЕГАИСПереопределяемый.ВидыЛицензийАлкогольнойПродукции()];
	...
	ТипыАП.Вставить(ПеречислениеВидыЛицензий.Спирт, "Спирт");
	ТипыАП.Вставить(ПеречислениеВидыЛицензий.СпиртосодержащаяНеПищеваяПродукция, "ССНП");
	ТипыАП.Вставить(ПеречислениеВидыЛицензий.СпиртосодержащаяПищеваяПродукция, "ССП");
	...
	|	ЕСТЬNULL(ВидыАлкогольнойПродукции."+ИнтеграцияЕГАИСПереопределяемый.ИмяКодаВидаПродукции()+", """") КАК КодВидаПродукции,
	|	ЕСТЬNULL(ВидыАлкогольнойПродукции.ВидЛицензии, ЗНАЧЕНИЕ(Перечисление."+ИнтеграцияЕГАИСПереопределяемый.ВидыЛицензийАлкогольнойПродукции()+".ПустаяСсылка)) КАК ВидЛицензии
	|ИЗ
...
Функция ПреобразоватьВXDTOАктПостановкиНаБаланс_v2(ДокументСсылка, ТекстОшибки)
	...
	Для Каждого СтрокаТовары Из ДанныеДляВыгрузки.ТаблицаТоваров Цикл
		...
		СправкаАXDTO = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип("http://fsrar.ru/WEGAIS/ActInventoryF1F2Info", "InformF1RegType"));
		УстановитьЗначениеСвойстваXDTO(СправкаАXDTO, "Quantity"    , СтрокаТовары.КоличествоПоСправкеА, ТекстОшибки);
		УстановитьЗначениеСвойстваXDTO(СправкаАXDTO, "BottlingDate", СтрокаТовары.ДатаРозлива         , ТекстОшибки);
		УстановитьЗначениеСвойстваXDTO(СправкаАXDTO, "TTNNumber"   , СтрокаТовары.НомерТТН            , ТекстОшибки);
		УстановитьЗначениеСвойстваXDTO(СправкаАXDTO, "TTNDate"     , СтрокаТовары.ДатаТТН             , ТекстОшибки);
		...
		ДанныеСправки1XDTO.InformF1 = СправкаАXDTO;
...
Функция ПреобразоватьВXDTOАктСписания(ДокументСсылка, ТекстОшибки)
	...
	УстановитьЗначениеСвойстваXDTO(ШапкаXDTO, "ActNumber"   , ДанныеДляВыгрузки.Номер                  , ТекстОшибки);
	УстановитьЗначениеСвойстваXDTO(ШапкаXDTO, "ActDate"     , ДанныеДляВыгрузки.Дата                   , ТекстОшибки);
...
Функция ПреобразоватьВXDTOАктСписания_v2(ДокументСсылка, ТекстОшибки)
	...
	Для Каждого СтрокаТовары Из ДанныеДляВыгрузки.ТаблицаТоваров Цикл
		СправкаБXDTO = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип("http://fsrar.ru/WEGAIS/ProductRef_v2", "InformF2TypeItem"));
		УстановитьЗначениеСвойстваXDTO(СправкаБXDTO, "F2RegId", СтрокаТовары.НомерСправкиБ, ТекстОшибки);
		
		ДанныеСправокXDTO = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "InformF1F2"));
		ДанныеСправокXDTO.InformF2 = СправкаБXDTO;
...
//
Функция ПреобразоватьВXDTOТТН(ДокументСсылка, ФорматОбмена, ТекстОшибки)
	...
	Иначе
		ТТНXDTO = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "WayBillType_v3"));
	КонецЕсли;
	...
	Для Каждого СтрокаТовары Из ДанныеДляВыгрузки.ТаблицаТоваров Цикл
		...
		НомерСправкиБ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтрокаТовары.СправкаБ, "РегистрационныйНомер");
		
		Если ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V3 Тогда
			УстановитьЗначениеСвойстваXDTO(ЭлементСодержимогоXDTO, "FARegId", СтрокаТовары.НомерСправкиА, ТекстОшибки);
			
			СправкаБXDTO = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип("http://fsrar.ru/WEGAIS/CommonV3", "InformF2TypeItemBC"));
			УстановитьЗначениеСвойстваXDTO(СправкаБXDTO, "F2RegId", НомерСправкиБ, ТекстОшибки);
		Иначе
			СправкаАXDTO = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип("http://fsrar.ru/WEGAIS/ProductRef_v2", "InformF1Type"));
			УстановитьЗначениеСвойстваXDTO(СправкаАXDTO, "RegId", СтрокаТовары.НомерСправкиА, ТекстОшибки);
			
			ЭлементСодержимогоXDTO.InformF1 = СправкаАXDTO;
			
			СправкаБXDTO = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип("http://fsrar.ru/WEGAIS/ProductRef_v2", "InformF2Type"));
		
			ЭлементСправкиБXDTO = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип("http://fsrar.ru/WEGAIS/ProductRef_v2", "InformF2TypeItem"));
			УстановитьЗначениеСвойстваXDTO(	ЭлементСправкиБXDTO, "F2RegId", НомерСправкиБ, ТекстОшибки);
			
			СправкаБXDTO.InformF2Item = ЭлементСправкиБXDTO;
		КонецЕсли;
		
		ЭлементСодержимогоXDTO.InformF2 = СправкаБXDTO;
...
// Функция возвращает объект XDTO, соответствующий документу ТТН.
//
Функция ПреобразоватьВXDTOТТН_v1(ДокументСсылка, ТекстОшибкиГлобальный)
...
//
Функция ПреобразоватьВXDTOАктПодтвержденияТТН(ВидДокумента, ФорматОбмена, ДокументСсылка, ТекстОшибкиГлобальный)
	...
	Иначе
		АктXDTO = ФабрикаXDTO.Создать(ФабрикаXDTO.Тип(ПространствоИмен, "WayBillActType_v3"));
	КонецЕсли;
...
// Функция возвращает объект XDTO, соответствующий акту подтверждения ТТН.
//
Функция ПреобразоватьВXDTOАктПодтвержденияТТН_v1(ВидДокумента, ДокументСсылка, ТекстОшибкиГлобальный)
...
Функция ПреобразоватьВXDTOИнформациюОФорматеОбмена(ПараметрыЗапроса, ТекстОшибки)
	...
	ИначеЕсли ПараметрыЗапроса.ФорматОбмена = Перечисления.ФорматыОбменаЕГАИС.V3 Тогда
		УстановитьЗначениеСвойстваXDTO(ИнформацияXDTO, "WBTypeUsed", "WayBill_v3", ТекстОшибки);
	КонецЕсли;
...
Функция ОбработатьВходящиеДокументы(МассивДокументов, ДополнительныеПараметры) Экспорт
	...
	Для Каждого СтруктураДокумента Из МассивДокументов Цикл
		...
		Если ОбъектXDTO.Document.TTNInformBReg <> Неопределено
			ИЛИ ОбъектXDTO.Document.TTNInformF2Reg <> Неопределено Тогда
			
			НовыеСправки = СправкиБ.СкопироватьКолонки();
			
			Если ОбъектXDTO.Document.TTNInformBReg <> Неопределено Тогда
		...
		ИначеЕсли ОбъектXDTO.Document.ActInventoryInformBReg <> Неопределено
			ИЛИ ОбъектXDTO.Document.ActInventoryInformF2Reg <> Неопределено Тогда
			
			Если ОбъектXDTO.Document.ActInventoryInformBReg <> Неопределено Тогда
		...
		ИначеЕсли ОбъектXDTO.Document.WayBill <> Неопределено
			ИЛИ ОбъектXDTO.Document.WayBill_v2 <> Неопределено
			ИЛИ ОбъектXDTO.Document.WayBill_v3 <> Неопределено Тогда
			
			Если ОбъектXDTO.Document.WayBill <> Неопределено Тогда
				ДанныеТТН = ПолучитьДанныеТТНВходящей(ОбъектXDTO.Document.WayBill, Перечисления.ФорматыОбменаЕГАИС.V1, ДополнительныеПараметры, РезультатОбработкиДокумента.ОписаниеОшибки);
			ИначеЕсли ОбъектXDTO.Document.WayBill_v2 <> Неопределено Тогда
				ДанныеТТН = ПолучитьДанныеТТНВходящей(ОбъектXDTO.Document.WayBill_v2, Перечисления.ФорматыОбменаЕГАИС.V2, ДополнительныеПараметры, РезультатОбработкиДокумента.ОписаниеОшибки);
			Иначе
				ДанныеТТН = ПолучитьДанныеТТНВходящей(ОбъектXDTO.Document.WayBill_v3, Перечисления.ФорматыОбменаЕГАИС.V3, ДополнительныеПараметры, РезультатОбработкиДокумента.ОписаниеОшибки);
			КонецЕсли;
		...
		ИначеЕсли ОбъектXDTO.Document.WayBillAct <> Неопределено
			ИЛИ ОбъектXDTO.Document.WayBillAct_v2 <> Неопределено
			ИЛИ ОбъектXDTO.Document.WayBillAct_v3 <> Неопределено Тогда
			
			Если ОбъектXDTO.Document.WayBillAct <> Неопределено Тогда
				РезультатОперации = ЗагрузитьАктПодтвержденияТТН(ОбъектXDTO.Document.WayBillAct, ДополнительныеПараметры);
			ИначеЕсли ОбъектXDTO.Document.WayBillAct_v2 <> Неопределено Тогда
				РезультатОперации = ЗагрузитьАктПодтвержденияТТН(ОбъектXDTO.Document.WayBillAct_v2, ДополнительныеПараметры);
			Иначе
				РезультатОперации = ЗагрузитьАктПодтвержденияТТН(ОбъектXDTO.Document.WayBillAct_v3, ДополнительныеПараметры);
			КонецЕсли;
		...
		ИначеЕсли ОбъектXDTO.Document.ConfirmTicket <> Неопределено Тогда
		...
		ИначеЕсли ОбъектXDTO.Document.Ticket <> Неопределено Тогда
			...
			Если ОбъектXDTO.Document.Ticket.Result <> Неопределено Тогда
			...
			ИначеЕсли ОбъектXDTO.Document.Ticket.OperationResult <> Неопределено Тогда
		...
		ИначеЕсли ОбъектXDTO.Document.ReplyClient <> Неопределено
			ИЛИ ОбъектXDTO.Document.ReplyClient_v2 <> Неопределено Тогда
		...
		ИначеЕсли ОбъектXDTO.Document.ReplyAP <> Неопределено
			ИЛИ ОбъектXDTO.Document.ReplyAP_v2 <> Неопределено Тогда
		...
		ИначеЕсли ОбъектXDTO.Document.ReplyFormA <> Неопределено
			ИЛИ ОбъектXDTO.Document.ReplyForm1 <> Неопределено Тогда
			
			ТекстОшибки = "";
			Если ОбъектXDTO.Document.ReplyFormA <> Неопределено Тогда
		...
		ИначеЕсли ОбъектXDTO.Document.ReplyFormB <> Неопределено
			ИЛИ ОбъектXDTO.Document.ReplyForm2 <> Неопределено Тогда
			
			ТекстОшибки = "";
			Если ОбъектXDTO.Document.ReplyFormB <> Неопределено Тогда
		...
		ИначеЕсли ОбъектXDTO.Document.ReplyRests <> Неопределено
			ИЛИ ОбъектXDTO.Document.ReplyRests_v2 <> Неопределено Тогда
			
			ДанныеЗапроса = РегистрыСведений.ПротоколОбменаЕГАИС.НайтиИсходящийЗапрос(СтруктураДокумента.ИдентификаторЗапроса);
			Если ДанныеЗапроса <> Неопределено И ЗначениеЗаполнено(ДанныеЗапроса.ДокументОснование) Тогда
				ТекстОшибки = "";
				Если ОбъектXDTO.Document.ReplyRests_v2 <> Неопределено Тогда
		...
		ИначеЕсли ОбъектXDTO.Document.ReplyRestsShop_v2 <> Неопределено Тогда
		...
		ИначеЕсли ОбъектXDTO.Document.TTNHistoryF2Reg <> Неопределено Тогда
...
Функция ЗагрузитьОрганизацию(ДанныеОрганизацииXDTO, ДополнительныеПараметры, ТекстОшибки)
	...
	Если ПустаяСтрока(Организация.Наименование) Тогда
		Организация.Наименование = ?(ПустаяСтрока(ЗагружаемаяОрганизацияXDTO.ShortName), ЗагружаемаяОрганизацияXDTO.FullName, ЗагружаемаяОрганизацияXDTO.ShortName);
	КонецЕсли;
...
Функция ЗагрузитьСписокАлкогольнойПродукции(СписокАлкогольнойПродукцииXDTO, ДополнительныеПараметры, ТекстОшибки)
	...
	ИмяКлассификатораВидов = СправочникВидовАлкогольнойПродукции();
	...
	|	ЕСТЬNULL(ВидыАлкогольнойПродукцииЕГАИС.Ссылка, ЗНАЧЕНИЕ(Справочник." + ИмяКлассификатораВидов + ".ПустаяСсылка)) КАК Ссылка
	|ИЗ
	|	КлассификаторВидовПродукции КАК КлассификаторВидовПродукции
	|		ЛЕВОЕ СОЕДИНЕНИЕ Справочник." + ИмяКлассификатораВидов + " КАК ВидыАлкогольнойПродукцииЕГАИС
	...
	ТаблицаАлкогольнойПродукции.Колонки.Добавить("ВидПродукции"      , Новый ОписаниеТипов("СправочникСсылка." + ИмяКлассификатораВидов));
	...
	ВидыЛицензий.Вставить("АП", ПеречислениеВидыЛицензий.АлкогольнаяПродукция);
	ВидыЛицензий.Вставить("ССП", ПеречислениеВидыЛицензий.СпиртосодержащаяПищеваяПродукция);
	ВидыЛицензий.Вставить("ССНП", ПеречислениеВидыЛицензий.СпиртосодержащаяНеПищеваяПродукция);
	ВидыЛицензий.Вставить("Спирт", ПеречислениеВидыЛицензий.Спирт);
	...
	Для Каждого ДанныеАлкогольнойПродукцииXDTO Из СписокАлкогольнойПродукцииXDTO Цикл
		...
		Если СтрокаКлассификатора <> Неопределено Тогда
			Если СтрокаКлассификатора.Ссылка.Пустая() Тогда
				ВидПродукции = Справочники[ИмяКлассификатораВидов].СоздатьЭлемент();
				ВидПродукции[ИмяКодаВида] = СтрокаКлассификатора.Код;
	...
	|					И НЕ ТаблицаАлкогольнойПродукции.ВидПродукции = ЗНАЧЕНИЕ(Справочник." + ИмяКлассификатораВидов + ".ПустаяСсылка)
	|				ИЛИ НЕ ТаблицаАлкогольнойПродукции.ТипПродукции = КлассификаторАлкогольнойПродукцииЕГАИС.ТипПродукции
	|					И НЕ ТаблицаАлкогольнойПродукции.ТипПродукции = ЗНАЧЕНИЕ(Перечисление.ТипыПродукцииЕГАИС.ПустаяСсылка)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
...
//
Функция ПолучитьДанныеТТНВходящей(ТТНXDTO, ФорматОбмена, ДополнительныеПараметры, ТекстОшибки)

	ДанныеТТН = ИнтеграцияЕГАИСКлиентСервер.СтруктураДанныхТТН();
	ДанныеТТН.Идентификатор = Строка(ТТНXDTO.Identity);
	ДанныеТТН.ФорматОбмена  = ФорматОбмена;
	
	ДанныеТТН.ВидОперации = ?(Найти(ВРег(ТТНXDTO.Header.Type), ВРег("Return")) <> 0,
	...
	Для Каждого ЭлементСодержимогоXDTO Из ТТНXDTO.Content.Position Цикл
		...
		ИначеЕсли ЕстьСвойствоОбъектаXDTO(ЭлементСодержимогоXDTO, "InformF1") Тогда
		...
		Иначе
			СтрокаТТН.НомерСправкиА = ЭлементСодержимогоXDTO.FARegId;
			СтрокаТТН.НомерСправкиБПоставщика = ЭлементСодержимогоXDTO.InformF2.F2RegId;
		КонецЕсли;
		...
		ИначеЕсли ЕстьСвойствоОбъектаXDTO(ЭлементСодержимогоXDTO, "InformF2")
...
Процедура УстановитьЗначениеСвойстваXDTO(ОбъектXDTO, ИмяСвойства, ЗначениеСвойства, ТекстОшибкиГлобальный, Глубина = Неопределено)
	...
	Исключение
		...
		ТекстОшибкиГлобальный = ТекстОшибкиГлобальный + ТекстОшибки + Символы.ПС;
	КонецПопытки;
...
Функция ПредставлениеОшибкиXDTO(ПредставлениеОшибки, ЧтениеXML, Глубина = Неопределено)
	...
	Если Найти(ПредставлениеОшибки, НСтр("ru = 'Ошибка проверки данных XDTO'")) = 0
		И Найти(ПредставлениеОшибки, НСтр("ru = 'Несоответствие типов XDTO'")) = 0 Тогда
...
Функция СформироватьУникальныйИдентификатор(ИсходнаяСтрока, ИмяСправочника)
	...
	ХешСумма = СтроковыеФункцииКлиентСервер.ВычислитьХешСтрокиПоАлгоритмуMD5(Префиксы[ИмяСправочника] + ИсходнаяСтрока);
