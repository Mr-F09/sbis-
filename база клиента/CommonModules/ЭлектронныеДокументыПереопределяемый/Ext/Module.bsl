...
Функция ТекстЗапросаШапка_УПД()
	...
	|	УПД.ВалютаДокумента.НаименованиеПолное КАК ВалютаНаименование,
	|	УПД.КурсВзаиморасчетов КАК Курс,
	|	УПД.КратностьВзаиморасчетов КАК Кратность,
	|	УПД.УчитыватьНДС,
	|	УПД.СуммаВключаетНДС,
	|	УПД.ВалютаДокумента.Код КАК ВалютаКод
	|ИЗ
...
Функция ПолучитьПараметрыФормыСопоставленияНоменклатуры(СсылкаНаЭД) Экспорт
	...
	Если (СсылкаНаЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Входящий
		 ...
		 ИЛИ (СсылкаНаЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Входящий 
...
//
// Возвращаемое значение:
//  Строка - Значение ставки НДС строкой ( 0% | 10% | 18% | 10/110 | 18/118 | без НДС )
//
Функция ПолучитьСтавкуНДССтрокой(Знач СтавкаНДС) Экспорт
	...
	ИначеЕсли СтавкаНДС = СтавкиНДС.НДС20_120 Тогда
		Результат = "20/120";
	ИначеЕсли СтавкаНДС = СтавкиНДС.НДС22_122 Тогда
		Результат = "22/122";
	Иначе
...
Процедура НайтиСоздатьУниверсальныйПередаточныйДокумент(ДеревоДанных, СсылкиНаВладельцев = Неопределено,
	...
	ПервичныйДокумент = СсылкиНаВладельцев.ПервичныйДокумент;
	СчетФактура = СсылкиНаВладельцев.СчетФактура;
