...
Функция ПолучитьДокументыОбОтгрузке(СсылкаНаОбъект)

Если СсылкаНаОбъект.Дата >= Дата(2024,10,1) тогда
	ТекСтрока = "Универсальный передаточный документ, № " + СокрЛП(ОбщегоНазначения.ПолучитьНомерНаПечать(СсылкаНаОбъект)) + " от " + СокрЛП(Формат(СсылкаНаОбъект.Дата,"ДФ=dd.MM.yyyy")) + " г.";
	Возврат ТекСтрока;
Иначе
	
КоличествоСтрок = СсылкаНаОбъект.Товары.Количество() + СсылкаНаОбъект.Услуги.Количество();
...
Если КоличествоСтрок = 1 тогда
  ПечКоличествоСтрок = "1";	
Иначе
  ПечКоличествоСтрок = "1-" + СокрЛП(КоличествоСтрок);	
КонецЕсли;	

ПечДокумент = " №" + СокрЛП(ОбщегоНазначения.ПолучитьНомерНаПечать(СсылкаНаОбъект)) + " от " + СокрЛП(Формат(СсылкаНаОбъект.Дата, "ДФ=dd.MM.yyyy")) + " г.";

Возврат "№ п/п " + ПечКоличествоСтрок + ПечДокумент;
...
Процедура ВывестиСчетФактуруВТабличныйДокумент(ТабДокумент, Макет, ДанныеДляПечати) Экспорт
	...
	Если Дата >= Дата(2021,7,1) тогда
	  ОбластьМакета.Параметры.ДокументыОбОтгрузке = ПолучитьДокументыОбОтгрузке(ДанныеДляПечати.ДокументОснование);
	КонецЕсли;	
	...
	Для Каждого Строчка Из ВыборкаСтрокТовары Цикл
		...
		Если ДанныеДляПечати.ВидСчетаФактуры = Перечисления.ВидСчетаФактурыВыставленного.НаРеализацию И 
			(Строчка.СтавкаНДС = Перечисления.СтавкиНДС.НДС20_120 ИЛИ Строчка.СтавкаНДС = Перечисления.СтавкиНДС.НДС22_122 ИЛИ Строчка.СтавкаНДС = Перечисления.СтавкиНДС.НДС5_105
...
Процедура ВывестиКорректировочныйСчетФактуруВТабличныйДокумент(ТабДокумент, Макет, ДанныеДляПечати) Экспорт
	...
	Для Каждого СтрокаТабличнойЧасти ИЗ ВыборкаТабличнойЧасти Цикл 
		...
		Если СтрокаТабличнойЧасти.СтавкаНДС = Перечисления.СтавкиНДС.НДС20_120 ИЛИ СтрокаТабличнойЧасти.СтавкаНДС = Перечисления.СтавкиНДС.НДС22_122 ИЛИ СтрокаТабличнойЧасти.СтавкаНДС = Перечисления.СтавкиНДС.НДС5_105
...
Функция ПечатьУниверсальногоПередаточногоДокумента(ДанныеДокумента, СчетФактураТребуется, СтруктураОтбораСчетаФактуры = Неопределено, ТабДокумент = Неопределено) Экспорт
	...
	Если ДанныеДокумента.Дата >= Дата(2021,7,1) тогда
	  Макет = ПолучитьОбщийМакет("УниверсальныйПередаточныйДокумент534");
	КонецЕсли;	
	
	Если ДанныеДокумента.Дата >= Дата(2024,10,1) тогда
	  Макет = ПолучитьОбщийМакет("УниверсальныйПередаточныйДокумент1096");
	КонецЕсли;	
	
	Если ДанныеДокумента.Дата >= Дата(2026,1,1) тогда
	  Макет = ПолучитьОбщийМакет("УниверсальныйПередаточныйДокумент2026");
	КонецЕсли;	

	Если ДанныеДокумента.Дата >= Дата(2026,4,1) тогда
	  Макет = ПолучитьОбщийМакет("УниверсальныйПередаточныйДокумент26");
	КонецЕсли;	
