...
Функция СтруктураДанныхТТН() Экспорт
	...
	Результат.Вставить("ФорматОбмена"      , Неопределено); // Значение перечисления ФорматыОбменаЕГАИС.
...
Функция СформироватьHTTPЗапрос(ТранспортныйМодуль, ДанныеЗапроса) Экспорт
	...
	Если НЕ ПустаяСтрока(ДанныеЗапроса.ТелоЗапроса) Тогда
		HTTPЗапрос.УстановитьТелоИзДвоичныхДанных(ИнтеграцияЕГАИСВызовСервера.ДвоичныеДанныеБезBOM(ДанныеЗапроса.ТелоЗапроса));
	КонецЕсли;
...
Функция ПодготовитьСписокURLДокументовКЗагрузке(ОтветМодуляЕГАИС) Экспорт
	...
	ФильтрURL = "/WAYBILL/,/WAYBILL_V2/,/WAYBILL_V3/,/FORMBREGINFO/,/FORM2REGINFO/,/WAYBILLACT/,/WAYBILLACT_V2/,/WAYBILLACT_V3/,/WAYBILLTICKET/";
...
//
Функция ЗавершитьОперациюСОшибкой(Результат = Неопределено, ОтображатьСообщения = Истина, ТекстОшибки = "") Экспорт
