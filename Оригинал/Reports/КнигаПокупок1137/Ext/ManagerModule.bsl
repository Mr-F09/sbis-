...
Функция ПолучитьДополнительнуюИнформациюПоСчетуФактуре(СтруктураПараметров)
	...
	"
	|ВЫБРАТЬ
	|	ВозвратТоваровОтПокупателя.Ссылка КАК СчетФактураДокумент,
	|	НЕОПРЕДЕЛЕНО КАК Продавец,
	|	NULL КАК ИНН,
	|	NULL КАК КПП,
	|	ВозвратТоваровОтПокупателя.ДатаРасходногоКассовогоОрдера КАК ДатаДокументаОплаты,
	|	ВозвратТоваровОтПокупателя.НомерРасходногоКассовогоОрдера КАК НомерДокументаОплаты,
	|	1 КАК КоличествоЭлементов,
	|	NULL КАК ЮридическоеФизическоеЛицо
	|ИЗ
	|	Документ.ВозвратТоваровОтПокупателя КАК ВозвратТоваровОтПокупателя
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ СписокСчетовФактур КАК СписокСчетовФактур
