...
Процедура ПриЗаписиЭлектронногоДокумента(Источник, Отказ) Экспорт
	...
	Если Источник.ДополнительныеСвойства.Свойство("СостояниеВерсииЭД") Тогда
		...
		Если ОбновитьВерсию Тогда
			...
			ЭлектронныеДокументыСлужебныйВызовСервера.ОбновитьВерсиюЭД(Источник.Ссылка, ПринудительнаяСменаСостоянияВерсии, ВерсияФорматаПакета, ОтклонениеАннулирования);
		КонецЕсли;
		...
		ИначеЕсли НастройкиОбмена <> Неопределено Тогда
			...
			Если НЕ НастройкиОбмена.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезВебРесурсБанка
				...
				Если (ТекущийСтатус = Перечисления.СтатусыЭД.Получен И ЭтоСчетНаОплату(СсылкаНаЭД))
						...
						Или (ЭтоОтветныйТитул(СсылкаНаЭД) И
							// последний статус для фнс документов через оператора
							(ТекущийСтатус = Перечисления.СтатусыЭД.ПереданОператору
						...
						Или (ТекущийСтатус = Перечисления.СтатусыЭД.ОтправленоПодтверждение
								И Не ЭтоCMLДокумент(СсылкаНаЭД))
			...
			Если МассивСтатусов.Количество() > 0 Тогда
				...
				Иначе
					...
					ИначеЕсли СледующийСтатус = Перечисления.СтатусыЭД.Отправлен
						И Не ЭлектронныеДокументыСлужебныйВызовСервера.ЭтоПрямойОбмен(НастройкиОбмена.СпособОбмена)
						И НЕ НастройкиОбмена.СпособОбмена = Перечисления.СпособыОбменаЭД.ЧерезВебРесурсБанка Тогда
...
Функция ЗаполнитьПараметрыЭДПоИсточнику(Источник, ФорматCML = Ложь, ВидЭД = Неопределено, БыстрыйОбмен = Ложь) Экспорт
	...
	Если ЭлектронныеДокументыСлужебныйВызовСервера.ИспользованиеУниверсальногоПередаточногоДокумента(ПараметрыЭД.Организация, ПараметрыЭД.Контрагент, ПараметрыЭД.ДоговорКонтрагента) Тогда
		...
		КонецЕсли;
	КонецЕсли;
	Если ЭлектронныеДокументыСлужебныйВызовСервера.ИспользованиеУниверсальногоКорректировочногоДокумента(ПараметрыЭД.Организация, ПараметрыЭД.Контрагент, ПараметрыЭД.ДоговорКонтрагента) Тогда
...
Процедура СохранитьПриглашения(ТаблицаПриглашений) Экспорт
	...
	Если Не Результат.Пустой() Тогда
		...
		Для Каждого НовыеПриглашения Из ТЗПоследнихИзменений Цикл
			...
			// Обработка ошибки поиска контрагента.
				...
				Если Выборка.СтатусПодключения = НовыеПриглашения.СтатусУчастника
					И Выборка.ИдентификаторКонтрагента = НовыеПриглашения.Идентификатор Тогда
						Если Выборка.ПометкаУдаления Тогда
							...
							НастройкаЭДО.УстановитьПометкуУдаления(Ложь);
...
Функция ПараметрыВыводаПредупреждения(ЭлектронныйДокумент) Экспорт
	...
	ПрисоединенныйФайлСсылка = ЭлектронныйДокумент;
...
Процедура ПроверитьКонтрагента(Параметры) Экспорт 
	...
	ПроверитьКонтрагентов(ДанныеКонтрагентов, Истина, Параметры.СохранятьРезультатСразуПослеПроверки, Параметры.Таймаут);
...
Процедура ПроверитьКонтрагентов(ДанныеКонтрагентов, ЭтоПроверкаСправочника = Истина, СохранятьРезультатСразуПослеПроверки = Ложь, Таймаут = Неопределено) Экспорт
	...
	ПолучитьРезультатПроверкиВебСервисом(ДанныеКонтрагентов, Отбор, Таймаут);
...
Процедура ПолучитьРезультатПроверкиВебСервисом(ДанныеКонтрагентов, Отбор, Таймаут)
	...
	Для НомерПорции = 1 По КоличествоЗапросов Цикл 
		...
		Соединение = ЭлектронныеДокументыВнутренний.ПолучитьСоединение(Перечисления.СпособыОбменаЭД.ЧерезСервис1СЭДО, Таймаут);
	...
	Если ЗначениеЗаполнено(СсылкаНаЭД) Тогда
		...
		Иначе
			РеквизитыЭД = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаЭД,
				"ВидЭД, НаправлениеЭД, СоглашениеЭД, ПрофильНастроекЭДО, ВерсияРегламентаЭДО, ПодписанЭЦП, ВладелецФайла, ТребуетсяПодтверждение, ТребуетсяИзвещение, ТипЭлементаВерсииЭД");
		КонецЕсли;
			
		Если ЗначениеЗаполнено(РеквизитыЭД) Тогда
			НастройкиОбмена = Новый Структура;
			
			Если РеквизитыЭД.Свойство("ВладелецФайла")
				И ТипЗнч(РеквизитыЭД.ВладелецФайла) = Тип("ДокументСсылка.ПроизвольныйЭД") Тогда
				
				ТребоватьПодтверждения = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(РеквизитыЭД.ВладелецФайла, "ТребуетсяПодтверждение");
...
Функция ВернутьМассивСтатусовЭД(НастройкиОбмена) Экспорт
	...
	Иначе
		Если НастройкиОбмена.Владелец().Колонки.Найти("ТребоватьПодтверждения") = Неопределено Тогда
			ТребуетсяПодтверждение = Истина;
		Иначе
			ТребуетсяПодтверждение = НастройкиОбмена.ТребоватьПодтверждения;
		КонецЕсли;
	КонецЕсли;
	...
	Если НастройкиОбмена.ВидЭД = Перечисления.ВидыЭД.ТОРГ12Покупатель
		...
		ИЛИ НастройкиОбмена.Свойство("ТипЭлементаВерсииЭД")
			И (НастройкиОбмена.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУПД
				ИЛИ НастройкиОбмена.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУКД) Тогда
	...
	Если НастройкиОбмена.Направление = Перечисления.НаправленияЭД.Исходящий Тогда
			...
			Иначе
				...
				Если НастройкиОбмена.ВерсияРегламентаЭДО = Перечисления.ВерсииРегламентаОбмена1С.Версия20 Тогда
					...
					Если НастройкиОбмена.ВидЭД <> Перечисления.ВидыЭД.ТОРГ12Покупатель
						...
						И НастройкиОбмена.ТипЭлементаВерсииЭД <> Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУПД
						И НастройкиОбмена.ТипЭлементаВерсииЭД <> Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУКД Тогда
				...
				Если НастройкиОбмена.ВидЭД = Перечисления.ВидыЭД.ПредложениеОбАннулировании Тогда
					МассивСтатусов.Добавить(Перечисления.СтатусыЭД.ПолученоПодтверждение);
				КонецЕсли;
		...
		Иначе
			...
			Иначе
				...
				ВерсияФорматаПакета = Неопределено;
				Если ТипЗнч(НастройкиОбмена) = Тип("Структура") Тогда
					НастройкиОбмена.Свойство("ВерсияФорматаПакета", ВерсияФорматаПакета);
				Иначе
					Если Не НастройкиОбмена.Владелец().Колонки.Найти("ВерсияФорматаПакета") = Неопределено Тогда
						ВерсияФорматаПакета = НастройкиОбмена.ВерсияФорматаПакета;
					КонецЕсли
				КонецЕсли;
				
				Если ВерсияФорматаПакета = Перечисления.ВерсииФорматаПакетаЭД.Версия30 Тогда
					...
					Если Не (НастройкиОбмена.ВидЭД = Перечисления.ВидыЭД.ТОРГ12Покупатель
						Или НастройкиОбмена.ВидЭД = Перечисления.ВидыЭД.АктЗаказчик
						Или НастройкиОбмена.ВидЭД = Перечисления.ВидыЭД.СоглашениеОбИзмененииСтоимостиПолучатель
						ИЛИ НастройкиОбмена.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУПД
						ИЛИ НастройкиОбмена.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУКД) Тогда
				...
				Иначе
					...
					ИспользоватьКвитанции = Неопределено;
					Если ТипЗнч(НастройкиОбмена) = Тип("Структура") Тогда
						НастройкиОбмена.Свойство("ИспользоватьКвитанции", ИспользоватьКвитанции);
					Иначе
						Если Не НастройкиОбмена.Владелец().Колонки.Найти("ИспользоватьКвитанции") = Неопределено Тогда
							ИспользоватьКвитанции = НастройкиОбмена.ИспользоватьКвитанции;
						КонецЕсли;
					КонецЕсли;
					
					Если ИспользоватьКвитанции = Истина Тогда
						МассивСтатусов.Добавить(Перечисления.СтатусыЭД.Доставлен);
					КонецЕсли;
					Если НастройкиОбмена.ИспользоватьПодпись
						...
						И НастройкиОбмена.ВидЭД <> Перечисления.ВидыЭД.АктЗаказчик
						И НастройкиОбмена.ВидЭД <> Перечисления.ВидыЭД.СоглашениеОбИзмененииСтоимостиПолучатель
						И НастройкиОбмена.Свойство("ТипЭлементаВерсииЭД")
						И НастройкиОбмена.ТипЭлементаВерсииЭД <> Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУПД
						И НастройкиОбмена.ТипЭлементаВерсииЭД <> Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУКД Тогда
		...
		Если НЕ (НастройкиОбмена.ВидЭД = Перечисления.ВидыЭД.ИзвещениеОПолучении
			...
			Иначе
				...
				Иначе
					МассивСтатусов.Добавить(Перечисления.СтатусыЭД.Утвержден);
					Если НастройкиОбмена.ИспользоватьПодпись
						И НастройкиОбмена.Свойство("ТребуетсяПодтверждение") И НастройкиОбмена.ТребуетсяПодтверждение
					И НЕ (НастройкиОбмена.ВидЭД = Перечисления.ВидыЭД.ТОРГ12Покупатель
						Или НастройкиОбмена.ВидЭД = Перечисления.ВидыЭД.АктЗаказчик
						Или НастройкиОбмена.ВидЭД = Перечисления.ВидыЭД.СоглашениеОбИзмененииСтоимостиПолучатель
						ИЛИ НастройкиОбмена.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУПД
						ИЛИ НастройкиОбмена.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУКД) Тогда
...
Функция СформироватьПрисоединенныеФайлы(МассивОбъектов, ПараметрыОбмена, ДопПараметры = "") Экспорт
	...
	Для Каждого СтруктураОбмена Из МассивСтруктурОбмена Цикл
		...
		ДокументыДляОбновленияВладельца = Новый Массив;
		ДокументыДляОбновленияВладельца.Добавить(ВладелецЭД);
		Если СтруктураОбмена.СтруктураЭД.Свойство("МассивПервичныхДокументовУПДУКД") Тогда
			Для каждого Документ Из СтруктураОбмена.СтруктураЭД.МассивПервичныхДокументовУПДУКД Цикл
				ДокументыДляОбновленияВладельца.Добавить(Документ);
			КонецЦикла;
		КонецЕсли;
		
		ДополнительныеСвойства = Новый Структура;
		ДополнительныеСвойства.Вставить("ДокументыДляОбновленияВладельца", ДокументыДляОбновленияВладельца);
		СтруктураОбмена.СтруктураЭД.Вставить("ДополнительныеСвойства", ДополнительныеСвойства);
...
Процедура ИзменитьСостояниеЭД(ИзменяемыеДокументы, НовоеСостояние, ИзменяемыеСостояния = Неопределено) Экспорт
	...
	|ПОМЕСТИТЬ втИзменяемыеЭД
	|ИЗ
	|	РегистрСведений.СостоянияЭД КАК СостоянияЭД
	|ГДЕ
	|	СостоянияЭД.СсылкаНаОбъект В(&ИзменяемыеДокументы)
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|ИЗ
	|	РегистрСведений.СостоянияЭД КАК СостоянияЭД
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ втИзменяемыеЭД КАК втИзменяемыеЭД
	|		ПО СостоянияЭД.ЭлектронныйДокумент = втИзменяемыеЭД.ЭлектронныйДокумент";
	Запрос.УстановитьПараметр("ИзменяемыеДокументы", ДокументыИБ);
	...
	Пока Выборка.Следующий() Цикл
		...
		СостояниеЭД.СсылкаНаОбъект = Выборка.СсылкаНаОбъект;
				...
				Для Каждого СтруктураДанных Из МассивДвоичныхДанных Цикл
					...
					Если Не ЕстьОшибкаРаспаковки Тогда
						...
						Если СоответствиеФайлПараметры.Свойство("ДопДанные", ДополнительныеДанные)
							...
							КонецЕсли;
						КонецЕсли;
						Если ЗначениеЗаполнено(ТипДанных) И ТребуетсяИзвещение
...
Процедура ПроверитьНаличиеНовыхЭД() Экспорт
	...
	Если Не ПолучитьФункциональнуюОпцию("ИспользоватьЭлектронныеЦифровыеПодписи") Тогда
		Возврат;
	КонецЕсли;
	...
	Соединение = ЭлектронныеДокументыВнутренний.ПолучитьСоединение(СпособОбмена);
...
//  Соединение				 - Http соединение - необязательный параметр
// 
// Возвращаемое значение:
	...
	Если Не Отказ Тогда
		Попытка
			...
			ИначеЕсли УПДДокументОПередаче Тогда
				ЭлектронныеДокументыПереопределяемый.НайтиСоздатьУПДДокументОПередаче(СтрокаОбъекта.ЗначениеРеквизита, ДокументСсылка, Записывать);
