...
Процедура ОбновитьВерсиюЭД(ЭлектронныйДокумент, ПринудительнаяСменаСостоянияВерсии = Ложь, ВерсияФорматаПакета = Неопределено,
	...
	Если НЕ ЭтоСлужебныйДокумент(ЭлектронныйДокумент)
		Или ЭлектронныеДокументыСлужебный.ЭтоОтветныйТитул(ЭлектронныйДокумент) Тогда

		Если (СвойстваЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.СЧФДОПУПД
			И ЭлектронныйДокумент.ВидЭД = Перечисления.ВидыЭД.СчетФактура)
			
			ИЛИ (СвойстваЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.КСЧФДИСУКД
			И ЭлектронныйДокумент.ВидЭД = Перечисления.ВидыЭД.КорректировочныйСчетФактура)
			...
			И ЭлектронныйДокумент.ВидЭД = Перечисления.ВидыЭД.КорректировочныйСчетФактура) Тогда
			
			ИсходныйЭлектронныйДокумент = ЭлектронныйДокумент;
			Если ЭлектронныеДокументыСлужебный.ЭтоОтветныйТитул(ЭлектронныйДокумент) Тогда
				ИсходныйЭлектронныйДокумент = СвойстваЭД.ЭлектронныйДокументВладелец;
			КонецЕсли;
			...
			Запрос.УстановитьПараметр("ЭлектронныйДокумент", ИсходныйЭлектронныйДокумент);
			Выборка = Запрос.Выполнить().Выбрать();
					
			СостояниеВерсииЭД = ЭлектронныеДокументыСлужебный.ОпределитьСостояниеВерсииПоСтатусуЭД(ЭлектронныйДокумент, ВерсияФорматаПакета,
			...
			Если (СвойстваЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУПД
					Или СвойстваЭД.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУКД)
			...
			Пока Выборка.Следующий() Цикл
				...
				Если НаборЗаписей.Количество() <> 0 Тогда
					...
					Если НоваяЗаписьНабора.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ЗакрытПринудительно Тогда
						// Для УПД и УКД меняет состояние "Закрыт принудительно" на "Обмен завершен",
						// если для него пришла вторая подпись.
						Если Не ((СвойстваЭД.ВидЭД = Перечисления.ВидыЭД.СчетФактура
								Или СвойстваЭД.ВидЭД = Перечисления.ВидыЭД.КорректировочныйСчетФактура)
					...
					Если ЭлектронныйДокумент = НоваяЗаписьНабора.ЭлектронныйДокумент
						...
						Если НоваяЗаписьНабора.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ОбменЗавершенСИсправлением Тогда
		...
		Иначе
			...
			Если НаборЗаписей.Количество() <> 0 Тогда
				...
				Если НоваяЗаписьНабора.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ЗакрытПринудительно Тогда
					// Для акта на передачу прав меняет состояние "Закрыт принудительно" на "Обмен завершен",
					// если для него пришла вторая подпись.
				...
				Если ЭлектронныйДокумент = НоваяЗаписьНабора.ЭлектронныйДокумент
					...
					Если НоваяЗаписьНабора.СостояниеВерсииЭД = Перечисления.СостоянияВерсийЭД.ОбменЗавершенСИсправлением
	...
	ИначеЕсли СвойстваЭД.ВидЭД = Перечисления.ВидыЭД.ИзвещениеОПолучении Тогда
		...
		|	РегистрСведений.СостоянияЭД КАК СостоянияЭД
		|ГДЕ
		|	СостоянияЭД.ЭлектронныйДокумент В
		|			(ВЫБРАТЬ
		|				ВТ_ЭлектронныеДокументы.ЭлектронныйДокумент
		|			ИЗ
		|				ВТ_ЭлектронныеДокументы)";
		Запрос.УстановитьПараметр("СсылкаНаОбъект", СвойстваЭД.ВладелецФайла);
		Выборка = Запрос.Выполнить().Выбрать();
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ЭлектронныеДокументыСлужебный.ЭтоТитулФНС(НоваяЗаписьНабора.ЭлектронныйДокумент) Тогда
				ОтветныйТитул = ЭлектронныеДокументыСлужебный.ВторойТитулДокумента(НоваяЗаписьНабора.ЭлектронныйДокумент);
				Если ЗначениеЗаполнено(ОтветныйТитул) Тогда
					СостояниеВерсииЭД = ЭлектронныеДокументыСлужебный.ОпределитьСостояниеВерсииПоСтатусуЭД(ОтветныйТитул);
				Иначе
					СостояниеВерсииЭД = ЭлектронныеДокументыСлужебный.ОпределитьСостояниеВерсииПоСтатусуЭД(
...
//
Процедура УстановитьНовуюВерсиюЭД(СсылкаНаОбъект, ПрисоединенныйФайл = Неопределено, УдалятьСтаруюВерсию = Ложь, Знач ОбновитьОснованияЭД = Истина) Экспорт
	...
	Если ОбновитьОснованияЭД И СтарыйЭлектронныйДокумент <> НоваяЗаписьНабора.ЭлектронныйДокумент Тогда
		...
		Для каждого Основание Из ОснованияЭД Цикл
			УстановитьНовуюВерсиюЭД(Основание, ПрисоединенныйФайл, Ложь, Ложь);
		КонецЦикла;
	КонецЕсли;
		...
		Если ПолучитьФайлДанныхДляОбработки(СтруктураДанных, ПапкаДляРасшифровки, ЭтоПроизвольныйЭД) Тогда
			...
			Если ЭтоПроизвольныйЭД <> Истина Тогда
				...
				Если ФайлыРасшифровки.Количество() > 0 Тогда
					...
					КодРегламента = "";
					Если СтруктураСлужебныхДанных.Свойство("КодРегламента", КодРегламента) И ЗначениеЗаполнено(КодРегламента) Тогда
			...
			Если СтруктураСлужебныхДанных.КодТранзакции = "CustomerInformation" Тогда
			...
			Иначе
				Попытка
					...
					Если НЕ ЗаписатьНовыйЭД И ЗначениеЗаполнено(ДобавленныйФайл.ВладелецФайла) Тогда
						СсылкаВладельцаЭД = ДобавленныйФайл.ВладелецФайла;
						ДокументСообщение = СсылкаВладельцаЭД.ПолучитьОбъект();
						СсылкаЭД = ДобавленныйФайл;
					...
					Иначе
						...
						ДокументСообщение.Текст           = ?(ДополнительныеДанные.Свойство("КомментарийЭД"), ДополнительныеДанные.КомментарийЭД, "");
						ДокументСообщение.Контрагент      = ПараметрыПакетаЭД.Контрагент;
						ДокументСообщение.Организация     = ПараметрыПакетаЭД.Организация;
						СсылкаВладельцаЭД = ДокументСообщение.ПолучитьСсылкуНового();
						СсылкаЭД = СсылкаНовогоЭД;
					...
					МассивИдентификаторовДокументовОснований = Неопределено;
					СтруктураПодписей.Свойство("МассивИдентификаторовДокументовОснований", МассивИдентификаторовДокументовОснований);
					УникальныйИДОснования = Неопределено;
					Если ТипЗнч(МассивИдентификаторовДокументовОснований) = Тип("Массив")
						И МассивИдентификаторовДокументовОснований.Количество() > 0 Тогда
						...
						ТЗПолученныхОснований = ЭлектронныеДокументыВнутренний.ТаблицаЗначенийДокументовОснованийПоИдентификаторам(
							МассивИдентификаторовДокументовОснований, СтруктураПараметровПоиска);
						НаборЗаписей = РегистрыСведений.ИдентификаторыДокументовОснованийВладельцевЭД.СоздатьНаборЗаписей();
						НаборЗаписей.Отбор.ЭлектронныйДокумент.Установить(СсылкаЭД);
						Для Каждого Идентификатор Из МассивИдентификаторовДокументовОснований Цикл
							...
							Иначе
								...
								НоваяЗапись.ЭлектронныйДокумент = СсылкаЭД;
								НоваяЗапись.ОснованиеПолучено = Ложь;
								НоваяЗапись.ДокументПотомок = СсылкаВладельцаЭД;
							КонецЕсли;
						КонецЦикла;
						НаборЗаписей.Записать(Истина);
					ИначеЕсли ЗначениеЗаполнено(ДопПараметры) И ДопПараметры.Свойство("УникальныйИДОснования", УникальныйИДОснования)
					...
					Если ЗначениеЗаполнено(ДобавленныйФайл) Тогда
						...
						Если СтруктураЭД.Свойство("ДополнительныеРеквизиты") Тогда
							...
							Если СтруктураЭД.ДополнительныеРеквизиты.Свойство("ПервичныйДокумент", ПервичныйДокументУПДУКД)
								...
								УстановитьНовуюВерсиюЭД(ПервичныйДокументУПДУКД, ДобавленныйФайл, Истина);
							КонецЕсли;
						КонецЕсли;
...
// Только для внутреннего использования
Процедура ПерезаполнитьДокументыИБПоЭД(ДокументИБ, Знач СсылкаНаЭД, ОбъектМетаданных = Неопределено, ДокументЗагружен = Ложь) Экспорт
	
	// Внимание! Для однократной сделки в ЭД будет Структура.
	
	ВходящийТитул = ВходящийТитул(СсылкаНаЭД);
	Если ВходящийТитул <> Неопределено Тогда
		СсылкаНаЭД = ВходящийТитул;
	КонецЕсли;
	
	ЗаполнитьДокументИБПоЭД(ДокументИБ, СсылкаНаЭД, ДокументЗагружен);
	...
	Если ЭлектронныеДокументыКлиентСервер.ЕстьДействие(Действия, "Утвердить") Тогда
		...
		Для Каждого ТекСтрока Из ТЗ_ЭД Цикл
			...
			Если ТекСтрока.СтатусЭД = Перечисления.СтатусыЭД.Сформирован ИЛИ ТекСтрока.СтатусЭД = Перечисления.СтатусыЭД.Получен Тогда
				...
				Если СвойстваЭД.ВидЭД = Перечисления.ВидыЭД.СчетФактура
...
Процедура ЗаполнитьКонтестЭДО(КонтекстЭДО) Экспорт
	
	КонтекстЭДО.АвторизацияНаСервере = ВыполнятьАвторизациюНаСервере();
	КонтекстЭДО.КриптографияНаСервере = ВыполнятьКриптооперацииНаСервере();
...
Процедура УстановитьПризнакУтвержден(ПрисоединенныйФайл, НовыйЭД)
	...
	Если РеквизитыЭД.ТребуетсяПодтверждение Тогда
		...
		Если ТранзакцияАктивна() И РеквизитыЭД.НаправлениеЭД = Перечисления.НаправленияЭД.Входящий
...
Процедура УдалитьНедоступныеДляФормированияЭДОбъекты(МассивСсылок)
	...
	|			КОГДА НЕ СостоянияЭД.ЭлектронныйДокумент.ВидЭД В (&ДокументыОбменаСБанками)
	|				ТОГДА СостоянияЭД.ЭлектронныйДокумент.СтатусЭД <> ЗНАЧЕНИЕ(Перечисление.СтатусыЭД.ОшибкаПередачи)
	...
	ДокументыОбменаСБанками = Новый Массив;
	ДокументыОбменаСБанками.Добавить(Перечисления.ВидыЭД.ВыпискаБанка);
	ДокументыОбменаСБанками.Добавить(Перечисления.ВидыЭД.ЗапросВыписки);
	ДокументыОбменаСБанками.Добавить(Перечисления.ВидыЭД.ЗапросНочнойВыписки);
	ДокументыОбменаСБанками.Добавить(Перечисления.ВидыЭД.Квитанция);
	
	Запрос.УстановитьПараметр("ДокументыОбменаСБанками", ДокументыОбменаСБанками);
...
Функция ОпределитьСводнуюИнформациюПоСтатусуЭД(СсылкаНаЭД) Экспорт
	...
	|СоглашениеЭД, ПрофильНастроекЭДО, ЭлектронныйДокументВладелец, ВладелецФайла, ТребуетсяИзвещение, ТребуетсяПодтверждение,
	|ТипЭлементаВерсииЭД");
...
Функция ОпределитьОбъектПривязки(СтруктураПараметровДокумента)
	...
	Если СтруктураПараметровДокумента.ВидЭД = Перечисления.ВидыЭД.ТОРГ12Покупатель
		...
		ИЛИ СтруктураПараметровДокумента.ВидЭД = Перечисления.ВидыЭД.СоглашениеОбИзмененииСтоимостиПолучатель
		ИЛИ СтруктураПараметровДокумента.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУПД
		ИЛИ СтруктураПараметровДокумента.ТипЭлементаВерсииЭД = Перечисления.ТипыЭлементовВерсииЭД.ИнформацияПокупателяУКД Тогда
...
Процедура ЗакрытьДокументыПринудительно(Знач МассивСсылокНаВладельцев, Знач ПричинаЗакрытия, КоличествоОбработанныхЭД) Экспорт
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТ_ЭлектронныеДокументы.СсылкаНаОбъект КАК СсылкаНаОбъект,
	|	ВТ_ЭлектронныеДокументы.ЭлектронныйДокумент КАК ЭлектронныйДокумент
	|ИЗ
	|	ВТ_ЭлектронныеДокументы КАК ВТ_ЭлектронныеДокументы
	|ГДЕ
	|	ВТ_ЭлектронныеДокументы.ЭлектронныйДокумент = ЗНАЧЕНИЕ(Справочник.ЭДПрисоединенныеФайлы.ПустаяСсылка)
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	КоличествоОбработанныхЭД = ТЗ.Количество();
	...
	ИначеЕсли ПараметрыЭД.ВидЭД = Перечисления.ВидыЭД.АктИсполнитель
		...
		Если ПараметрыЭД.ВидЭД = Перечисления.ВидыЭД.ТОРГ12Продавец
			...
			Если ПространствоИменСхемы = "TORGPR" Тогда
				...
				Если ДанныеФайлаЭД.Документ.СодФХЖ3.СвЛицОтпГруз = Неопределено Тогда
					ГрузОтпустил = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML(
					...
					ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ФИО, "Имя", ИмяСертификата, Истина, ТекстОшибки);
					Если ЗначениеЗаполнено(ОтчествоСертификата) Тогда
						ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ФИО, "Отчество", ОтчествоСертификата, Истина, ТекстОшибки);
					КонецЕсли;
					ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведенияЛица, "ФИО", ФИО, Истина, ТекстОшибки);
					ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ГрузОтпустил, "РабОргПрод", СведенияЛица, Истина, ТекстОшибки);
					ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ДанныеФайлаЭД.Документ.СодФХЖ3,
						"СвЛицОтпГруз", ГрузОтпустил, Истина, ТекстОшибки);
				КонецЕсли;
		...
		ИначеЕсли ПараметрыЭД.ВидЭД = Перечисления.ВидыЭД.ТОРГ12Покупатель
			...
			Если ПространствоИменСхемы = "TORGPOK" Тогда // Передача товаров, акт результатов работ (Исполнитель).
				ГрузПолучил = ЭлектронныеДокументыВнутренний.ПолучитьОбъектТипаCML(
				...
				КонецЕсли;
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(СведенияЛица, "ФИО", ФИО, Истина, ТекстОшибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ГрузПолучил, "РабОргПок", СведенияЛица, Истина, ТекстОшибки);
				ЭлектронныеДокументыВнутренний.ЗаполнитьСвойствоXDTO(ДанныеФайлаЭД.Документ.СодФХЖ4.ГрузПолучил,
					"СвЛицПолГруз", ГрузПолучил, Истина, ТекстОшибки);
			Иначе
		...
		Попытка
			...
			Если НЕ ЭтоИзвещение Тогда
				Если ВидЭД = Перечисления.ВидыЭД.УведомлениеОбУточнении Тогда
					...
					КонецЦикла;
				Иначе
					ТребуетсяПодтверждение = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СсылкаНаЭД, "ТребуетсяПодтверждение");
...
Функция МожноАннулироватьЭтотЭД(Знач СсылкаНаЭД) Экспорт
	...
	СвойстваЭД = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(СсылкаНаЭД, "ВидЭД, СтатусЭД, НаправлениеЭД, ТребуетсяПодтверждение");
...
Процедура НайтиДокументыОснования(НомераЭД, НаправлениеЭД, ОснованияИБ) Экспорт
	...
	|		И ЭДПрисоединенныеФайлыИдентификаторыОснованийВладельцаФайла.ИдентификаторДокументаОснования <> ЭДПрисоединенныеФайлы.НомерЭД 
	|ГДЕ
