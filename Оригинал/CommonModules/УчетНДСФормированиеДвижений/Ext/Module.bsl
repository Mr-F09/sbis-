...
Процедура СформироватьДвиженияПоРегиструНДСНачисленный_ОтражениеРеализации(СтруктураШапкиДокумента, ТабЧасть, Движения, Отказ, ЭтоВозврат = ложь,УчетнаяПолитикаНУ = Неопределено) Экспорт
	...
	Если ТаблицаДвижений_НДСНачисленный.Количество() > 0 Тогда
		ПереопределитьВидЦенностиДляНесырьевыхТоваров(ТаблицаДвижений_НДСНачисленный, СтруктураШапкиДокумента, Истина);
	...
	Если не ТаблицаДвижений_НДСРеализация0 = Неопределено Тогда
		ПереопределитьВидЦенностиДляНесырьевыхТоваров(ТаблицаДвижений_НДСРеализация0, СтруктураШапкиДокумента, Ложь);
		Если НЕ ТипЗнч(Движения) = Тип("Структура") тогда
...
Процедура ПереопределитьВидЦенностиДляНесырьевыхТоваров(ТаблицаРеализацийПоСтавке0, СтруктураШапкиДокумента, НДСНачисленный)
