...
Функция ПровестиИРаспечататьЧек(Ответ, Отказ, ФормаДокумента, РучнойРежимПечати = Ложь) Экспорт
	...
	ИспользуетсяРегистрацияВЕГАИС = ПолучитьФункциональнуюОпцию("ВестиСведенияДляДекларацийПоАлкогольнойПродукции") 
									И ИнтеграцияЕГАИСВызовСервера.ИспользуетсяРегистрацияРозничныхПродажВЕГАИС();
	...
	Иначе
		...
		|	ЕСТЬNULL(ТчАкцизныеМарки.ШтрихкодМарки, """") КАК ШтрихкодМарки,
		|	ВЫБОР
		...
		|		ИНАЧЕ ТчТовары.Номенклатура.ПроизводительИмпортерАлкогольнойПродукции
		...
		Если ИспользуетсяРегистрацияВЕГАИС Тогда
			ПараметрыПередачи = ИнтеграцияЕГАИСПереопределяемый.ПараметрыЧекаДляПодписиВЕГАИС(ТаблицаТоваров, СуммаНал, СуммаБезНал, Ссылка);
			ДопДанные.Вставить("ПараметрыПередачиВЕГАИС", ПараметрыПередачи);
		КонецЕсли;
	...
	Иначе
		...
		Если ДопДанные <> Неопределено И ТипЗнч(ДопДанные) = Тип("Структура") Тогда
			...
			Если ДопДанные.Свойство("ПараметрыПередачиВЕГАИС", ПараметрыПередачи) Тогда
				...
				ПараметрыПередачи[09] = НомерЧекаККМ;
				ПараметрыПередачи[10] = НомерСменыККМ;
				
				ИндексПоследнего = ПараметрыПередачи.Количество() - 1;
				Если ТипЗнч(ПараметрыПередачи[ИндексПоследнего]) = Тип("Структура") Тогда
					ТМ = ПараметрыПередачи[ИндексПоследнего];
				Иначе
					ТМ = Неопределено;
				КонецЕсли;
				...
				РезультатОперации = ИнтеграцияЕГАИСКлиент.СформироватьПодписьЧека(ПараметрыПередачи,ТМ,ПараметрыВозврата,,,Ложь);
				Если НЕ РезультатОперации.Результат Тогда
					МассивСтрокДляПечати = Новый Массив;
					МассивСтрокДляПечати.Добавить("Не удалось сформировать электронную подпись чека в ЕГАИС!");
					МассивСтрокДляПечати.Добавить(РезультатОперации.ОписаниеОшибки);
					ОбработкаОбслуживания.НапечататьСтроки(ОбъектДрайвера, МассивСтрокДляПечати, Неопределено);
					ПереданВЕГАИСУспешно = Ложь;
				Иначе
					ДопДанные.Вставить("ПараметрыВозвратаИзЕГАИС", ПараметрыВозврата);
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
