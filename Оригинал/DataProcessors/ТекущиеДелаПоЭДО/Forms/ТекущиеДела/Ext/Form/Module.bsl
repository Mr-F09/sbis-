...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если МассивДоступныхДокументов.Количество() > 0 Тогда
		...
		|	СостоянияЭД.СсылкаНаОбъект КАК КлючСчетчика,
		...
		Создать.ТекстЗапроса = ТекстЗапроса;
...
Процедура ИзменитьДокумент(Команда)
	...
	Если Элементы.Создать.ТекущиеДанные <> Неопределено Тогда
		ОткрытьЗначение(Элементы.Создать.ТекущиеДанные.Документ);
	КонецЕсли;
...
Процедура СформироватьПодписатьИОтправить(Команда)
	...
	МассивДокументов = МассивДокументов(Элементы.Создать.ВыделенныеСтроки);
...
Процедура УстановитьОтборПоОрганизации(Форма, Значение)
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(Форма.Создать.Отбор,
	...
	ОбщегоНазначенияКлиентСервер.УстановитьЭлементОтбора(Форма.ЖдемСогласия.Отбор,
		"Отправитель", Значение, ВидСравненияКомпоновкиДанных.Равно,, ИспользоватьОтбор);
...
&НаКлиентеНаСервереБезКонтекста
Процедура УстановитьОтборПоДополнительнойИнформацииВСписке(Отбор, ЗначениеОтбора, ИспользоватьОтбор)
	...
	ОбщегоНазначенияКлиентСервер.ДобавитьЭлементКомпоновки(ГруппаИли, ИмяПоля, ВидСравненияЭлементаОтбора, ПравоеЗначение,, ИспользоватьОтбор);
...
Процедура УстановитьОтборПоДополнительнойИнформации(Форма, ЗначениеОтбора)
	...
	УстановитьОтборПоДополнительнойИнформацииВСписке(Форма.Утвердить.Отбор, ЗначениеОтбора, ИспользоватьОтбор);
	УстановитьОтборПоДополнительнойИнформацииВСписке(Форма.Исправить.Отбор, ЗначениеОтбора, ИспользоватьОтбор);
	УстановитьОтборПоДополнительнойИнформацииВСписке(Форма.Подписать.Отбор, ЗначениеОтбора, ИспользоватьОтбор);
	УстановитьОтборПоДополнительнойИнформацииВСписке(Форма.Обработать.Отбор, ЗначениеОтбора, ИспользоватьОтбор);
	УстановитьОтборПоДополнительнойИнформацииВСписке(Форма.НаКонтроле.Отбор, ЗначениеОтбора, ИспользоватьОтбор);
	УстановитьОтборПоДополнительнойИнформацииВСписке(Форма.Аннулировать.Отбор, ЗначениеОтбора, ИспользоватьОтбор);
...
Функция ДанныеДереваДействий(Знач ДеревоДействий, Запросы)
	...
	ПредставлениеСтроки = НСтр("ru = 'Создать'");
	
	УстановитьПредставлениеСтрокДереваРекурсивно(Дерево, "Создать", ПредставлениеСтроки, ДанныеОКоличествах[0]);
...
Процедура УдалитьОтборыПоСумме()
	...
	ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбора(Создать.Отбор, "СуммаДокумента");
...
Процедура УдалитьОтборыПоДате()
	...
	ОбщегоНазначенияКлиентСервер.УдалитьЭлементыГруппыОтбора(Создать.Отбор, "Дата");
...
Функция ДанныеОКоличествах()
	...
	Если ТекстЗапросаСчетчиков = "" Тогда
		Для Каждого ТекстЗапроса Из Запросы Цикл
			...
			ТекстЗапроса = Символы.ПС + "ВЫБРАТЬ РАЗРЕШЕННЫЕ КОЛИЧЕСТВО (РАЗЛИЧНЫЕ ОсновнаяТаблица.Ссылка) КАК КОЛИЧЕСТВО" + 
				Символы.ПС + Сред(ТекстЗапроса, ПозицияИЗ);
